...
fn get_bound_points(lane: &Lane, num_points: usize, sampling_step: f64, side: BoundSide) -> Vec<Vec3D> {
    (0..num_points)
        .map(|i| {
            ...
            let inertial_pos: InertialPosition = lane.to_inertial_position(&lane_pos).expect("Failed to convert lane position to inertial position");

            // Convert to a Rerun Vec3D for logging
            Vec3D::new(inertial_pos.x() as f32, inertial_pos.y() as f32, inertial_pos.z() as f32)
        })
        .collect::<Vec<_>>()
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sampling_step = 0.1; // meters
    let lanes = rg.get_lanes();

    for lane in &lanes {
        println!("Lane ID: {}", lane.id());
        println!("Length: {}", lane.length());
        println!("Num Points: {}", (lane.length() / sampling_step) as usize);
